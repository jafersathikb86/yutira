...
export default function AdminPage() {
  ...
  return (
    <Section title="Admin Dashboard" subtitle="Search participants, verify payments, mark attendance, manage paper decisions.">
      <div className="card p-6">
        <div className="flex items-center justify-between gap-3">
          <div className="text-sm text-white/70">Logged in as admin</div>
          <button onClick={logout} className="px-3 py-2 rounded-xl border border-white/20 hover:bg-white/10 text-sm">Logout</button>
        </div>
        ...
        <div className="mt-4 flex flex-col md:flex-row gap-3">
          ...
          />
          <button onClick={search} className="px-4 py-3 rounded-2xl bg-white text-black font-semibold">Search</button>
          <a
          ...
          {results.map((u) => (
            <div key={u._id} className="card p-5">
              <div className="flex flex-col md:flex-row md:items-start justify-between gap-3">
                <div>
                  <div className="font-semibold">{u.name} <span className="text-white/60 text-sm">({u.yutiraId})</span></div>
              ...
              <div className="mt-4 grid md:grid-cols-3 gap-3 text-sm">
                ...
                <div className="card p-4">
                  ...
                  <div className="mt-2 text-xs text-white/70">Decision works if submission exists.</div>
